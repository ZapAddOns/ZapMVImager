...
namespace ZapMVImager.Objects
{
    public class LogEntry
    {
        ...
        public static Regex RegexSystemData = new Regex(@"(\d{2}.\d{2}.\d{4})\s(\d{2}:\d{2}:\d{2}\.\d{3}).*SystemDeliveryData\s*TreatmentType:>\s*(\w*),\s*isTreatment:>\s*(\w*),\s*MV:>\s*(\d*\.\d*)\s*MU,\s*Collimator:>\s*(\d*)\s*mm,\s*Axial:>\s*(\d*\.\d*),\s*Oblique:>\s*(\d*\.\d*),\s*PlanName:>\s*(.*)", RegexOptions.Compiled);
